...
func githubTeamCreate(d *schema.ResourceData, meta interface{}) error {
	id := githubTeamID(d.Get("backend").(string), d.Get("team").(string))
	d.SetId(id)
	d.MarkNewResource()
...
func githubTeamRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("backend", githubTeamBackendPath(d.Id()))
...
func githubTeamID(backend string, name string) string {
	return fmt.Sprintf("auth/%s/map/teams/%s", strings.Trim(backend, "/"), name)
}

func githubTeamBackendPath(teamId string) string {
	name := teamId[strings.LastIndex(teamId, "/")+1:]
	teamPath := "/map/teams/" + name
	s := strings.Replace(teamId, teamPath, "", -1)
