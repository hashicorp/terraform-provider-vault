...
func TestAccOktaAuthBackend_WriteOnlyTokenDeprecated(t *testing.T) {
	t.Parallel()
	path := acctest.RandomWithPrefix("tf-test-auth-okta-wo-dep")
	organization := "example"
	resourceName := "vault_okta_auth_backend.test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccOktaAuthConfig_writeOnlyTokenDeprecated(path, organization, 1),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, consts.FieldPath, path),
					resource.TestCheckResourceAttr(resourceName, consts.FieldTokenWOVersion, "1"),
					// token_wo should not be in state (write-only)
...
func TestAccOktaAuthBackend_WriteOnlyToken_DeprecatedConflicts(t *testing.T) {
	t.Parallel()
	path := acctest.RandomWithPrefix("tf-test-auth-okta-wo-dep-conflict")
	organization := "example"
...
resource "vault_okta_auth_backend" "test" {
	...
	api_token_wo = "write-only-token"
	api_token_wo_version = 1
	token_wo = "deprecated-write-only-token"
	token_wo_version = 1
}
`, path, organization),
				ExpectError: regexp.MustCompile("Conflicting configuration arguments|conflicts with"),
...
func testAccOktaAuthConfig_writeOnlyTokenDeprecated(path string, organization string, version int) string {
	return fmt.Sprintf(`
resource "vault_okta_auth_backend" "test" {
	description = "Testing the Terraform okta auth backend with deprecated write-only token"
	path = "%s"
	organization = "%s"
	token_wo = "this-is-deprecated-write-only-secret"
	token_wo_version = %d
}
`, path, organization, version)
}
