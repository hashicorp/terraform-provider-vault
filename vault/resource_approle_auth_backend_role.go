...
func approleAuthBackendRoleResource() *schema.Resource {
	...
	util.AddTokenFields(fields, &util.AddTokenFieldsConfig{
...
func approleAuthBackendRoleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if _, deprecated := d.GetOk("bound_cidr_list"); deprecated {
		// Then we try to see if `secret_id_bound_cidrs` is returned and then set it to
		// `bound_cidr_list`. Failing that, we check that `bound_cidr_list` is returned
