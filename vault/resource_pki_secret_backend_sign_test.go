...
func TestPkiSecretBackendSign_removeRootsFromChain(t *testing.T) {
	rootPath := "pki-root-" + strconv.Itoa(acctest.RandInt())
	intermediatePath := "pki-intermediate-" + strconv.Itoa(acctest.RandInt())
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testPkiSecretBackendSignConfig_removeRootsFromChain(rootPath, intermediatePath, false),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "backend", intermediatePath),
					resource.TestCheckResourceAttr(resourceName, "common_name", "cert.test.my.domain"),
			...
			{
				Config: testPkiSecretBackendSignConfig_removeRootsFromChain(rootPath, intermediatePath, true),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "backend", intermediatePath),
					resource.TestCheckResourceAttr(resourceName, "common_name", "cert.test.my.domain"),
...
func testPkiSecretBackendSignConfig_removeRootsFromChain(rootPath, intermediatePath string, removeRoots bool) string {
	return fmt.Sprintf(`
resource "vault_mount" "test-root" {
  path                      = "%s"
  type                      = "pki"
