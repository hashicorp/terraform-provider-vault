...
// TestPkiSecretBackendSign_Upgrade verifies upgrades do not force recreation of "vault_pki_secret_backend_sign" resources.
// This safeguards against unintended schema changes that would trigger resource replacement.
func TestPkiSecretBackendSign_Upgrade(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Config: testPkiSecretBackendSignConfig_upgrade(path),
			},
			{
				ProtoV5ProviderFactories: testAccProtoV5ProviderFactories(context.Background(), t),
				Config:                   testPkiSecretBackendSignConfig_upgrade(path),
...
func testPkiSecretBackendSignConfig_upgrade(path string) string {
	return fmt.Sprintf(`
