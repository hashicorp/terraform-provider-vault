...
* `unregistered_user_policies` - (Optional) A set of policies to be granted to
  unregistered users.

* `dial_timeout` - (Optional) Number of seconds to wait for connecting to the 
  RADIUS server. Defaults to `10`.

* `read_timeout` - (Optional) Number of seconds to wait for a response from the
  RADIUS server. Defaults to `10`.
...
```
$ terraform import vault_radius_auth_backend.example auth/radius/config
$ terraform import vault_radius_auth_backend.example auth/my-radius/config
```
