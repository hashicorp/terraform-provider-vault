...
- `aws_secret_access_key` - AWS secret access key.
  Deprecated: use `aws_secret_access_key_wo` instead.
  Conflicts with `aws_secret_access_key_wo`.

- `aws_secret_access_key_wo` - AWS secret access key (write-only).
  This value is sent to Vault but never stored in Terraform state. With Vault 2.2.0 
...
- `secrets_wo_version` - Version used with write-only secrets.
  If omitted, the provider auto-manages this value and increments it when
  write-only secret values change. If set, you manage version increments
  manually.
