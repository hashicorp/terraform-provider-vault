---
layout: "vault"
page_title: "Vault: vault_pki_external_ca_secret_backend_acme_account resource"
sidebar_current: "docs-vault-resource-pki-external-ca-secret-backend-acme-account"
description: |-
  Manages PKI External CA ACME accounts for external CA integration.
---

# vault\_pki\_external\_ca\_secret\_backend\_acme\_account
...
```hcl
resource "vault_pki_external_ca_secret_backend_acme_account" "with_eab" {
  mount         = vault_mount.pki-external-ca.path
...
```hcl
resource "vault_pki_external_ca_secret_backend_acme_account" "with_ca" {
  mount         = vault_mount.pki-external-ca.path
