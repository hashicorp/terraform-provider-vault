---
layout: "vault"
page_title: "Vault: vault_pki_external_ca_secret_backend_order_challenge_fulfilled resource"
sidebar_current: "docs-vault-resource-pki-external-ca-secret-backend-order-challenge-fulfilled"
description: |-
  Marks an ACME challenge as fulfilled for a specific identifier in an order.
---

# vault\_pki\_external\_ca\_secret\_backend\_order\_challenge\_fulfilled
...
resource "local_file" "challenge" {
  filename = "/var/www/html/.well-known/acme-challenge/${data.vault_pki_external_ca_secret_backend_order_challenge.example.token}"
  content  = data.vault_pki_external_ca_secret_backend_order_challenge.example.key_authorization
}
...
```
$ terraform import vault_pki_external_ca_secret_backend_order_challenge_fulfilled.example pki-external-ca/role/example-role/order/abc123/fulfilled-challenge/http-01/www.example.com
```
