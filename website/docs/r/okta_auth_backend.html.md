...
* `api_token` - (Optional) The Okta API token. This is required to query Okta for user group membership.
  ...
  Conflicts with `token`, `api_token_wo`, and `token_wo`.
...
* `token` - (Optional) **Deprecated: Use `api_token` instead.** The Okta API token. This is required to query Okta for user group membership.
  If this is not supplied only locally configured groups will be enabled.
  Conflicts with `api_token`, `api_token_wo`, and `token_wo`.
...
* `token_wo_version` - (Optional) **Deprecated: Use `api_token_wo_version` instead.** Version counter for the write-only `token_wo`.
  Increment this value to trigger an update of the write-only token. Required when using `token_wo`.
...
* `api_token_wo` - (Optional) Write-only Okta API token. This is required to query Okta for user group membership.
  Use this for enhanced security when you don't want the token to appear in state files. Requires `api_token_wo_version`. Conflicts with `token`, `api_token`, and `token_wo`.
  **Note**: This property is write-only and will not be read from the API.

* `token_wo` - (Optional) **Deprecated: Use `api_token_wo` instead.** Write-only Okta API token. This is required to query Okta for user group membership.
  Use this for enhanced security when you don't want the token to appear in state files. Requires `token_wo_version`. Conflicts with `token`, `api_token`, and `api_token_wo`.
  **Note**: This property is write-only and will not be read from the API.
