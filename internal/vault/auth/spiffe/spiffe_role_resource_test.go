		...
		Steps: []resource.TestStep{
			...
			// Test updating the role to have a different workload id patterns (workload id can't be empty)
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceAddress, "token_ttl", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_max_ttl", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_policies.#", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_bound_cidrs.#", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_explicit_max_ttl", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_no_default_policy", "false"),
					resource.TestCheckResourceAttr(resourceAddress, "token_num_uses", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_period", "0"),
					resource.TestCheckResourceAttr(resourceAddress, "token_type", "default"),
					resource.TestCheckResourceAttr(resourceAddress, "alias_metadata.%", "0"),
				),
			},
			...
			// Test that we flush back to a simpler role - token fields are computed so they retain previous values
			// except alias_metadata which Vault clears when not provided
			{
				Config: spiffeRoleConfig(mount, workloadIds),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet(resourceAddress, "mount"),
