...
func (s *SpiffeConfigResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	client, err := client.GetClient(ctx, s.Meta(), data.Namespace.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(errutil.ClientConfigureErr(err))
		return
	}

	vaultRequest, diagErr := getApiModel(ctx, data)
	if diagErr != nil {
	...
	_, err = client.Logical().WriteWithContext(ctx, s.path(data.Mount.ValueString()), vaultRequest)
	if err != nil {
...
// getApiModel translates the Terraform model into the Vault API model to send updates to Vault.
// As a normal JSON model can't differentiate between a missing field and a field set to a zero value,
// we manually build up the map to send to Vault.
func getApiModel(ctx context.Context, data SpiffeConfigModel) (map[string]any, diag.Diagnostics) {
	vaultRequest := map[string]any{
		"trust_domain":       data.TrustDomain.ValueString(),
		"profile":            data.Profile.ValueString(),
		"defer_bundle_fetch": data.DeferBundleFetch.ValueBool(),
	}
